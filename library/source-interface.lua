...
local ConfigurationExtension = ".priv"
...
function Hide()
	Debounce = true
	PrivilegedLibrary:Notify({Title = "Interface Oculta", Content = "Pressione 'K' para reabrir o painel.", Duration = 7})
...
function PrivilegedLibrary:CreateWindow(Settings)
	...
	if Settings.Discord then
		if not isfolder(PrivilegedFolder.."/Discord Invites") then
			makefolder(PrivilegedFolder.."/Discord Invites")
		end
		if not isfile(PrivilegedFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
	...
	if Settings.KeySystem then
		...
		if not isfolder(PrivilegedFolder.."/Key System") then
			makefolder(PrivilegedFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Privileged | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(PrivilegedFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(PrivilegedFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
...
local MinimizeBTNmbl_UserInputService = game:GetService("UserInputService")

MinimizeBTNmbl_ScreenGui.Parent = game:GetService("CoreGui")
MinimizeBTNmbl_ScreenGui.ResetOnSpawn = false

local MinimizeBTNmbl_sizeScale = 0.15
...
local function MinimizeBTNmbl_minimize_upd(input)
...
MinimizeBTNmbl_UserInputService.InputChanged:Connect(function(input)
    if input == MinimizeBTNmbl_dragInput and MinimizeBTNmbl_dragging then
        MinimizeBTNmbl_isMoved = true
        MinimizeBTNmbl_minimize_upd(input)
    end
end)
